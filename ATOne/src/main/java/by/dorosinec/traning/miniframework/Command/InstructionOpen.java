...
/**
 * Class of executing instruction "open"
 */
public class InstructionOpen extends InstructionImplementation {
    ...
    public boolean canExecution(Instruction instruction) {
        return instruction.getNameCommand().equals(Instruction.OPEN);
    }

    /**
     * Method which executing instruction "open"
     *
     * @param instruction instruction to be executed
    ...
    public InstructionFinal commandExecution(Instruction instruction, WebDriver driver) throws Exception {
        ...
        driver.get(instruction.getArg1());
        BigDecimal secondTime = new BigDecimal(System.currentTimeMillis());
        boolean flag = getRunTime(firstTime, secondTime).compareTo(new BigDecimal(instruction.getArg2())) == -1;
